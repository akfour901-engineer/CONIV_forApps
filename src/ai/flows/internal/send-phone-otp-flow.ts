...
/**
 * @fileOverview A flow to generate and send an email OTP for user verification.
 */
import { z } from 'zod';
...
const SendEmailOtpInputSchema = z.object({
  email: z.string().email(),
});

const SendEmailOtpOutputSchema = z.object({
  success: z.boolean(),
  message: z.string(),
});
...
export const sendEmailOtpFlow = ai.defineFlow(
  ...
  async ({ email }) => {
    ...
    try {
      ...
      // If the above line doesn't throw, a user with this email already exists.
      return { success: false, message: "An account with this email already exists. Please sign in." };
    } catch (error: any) {
      // "auth/user-not-found" is the expected error if the user is new.
      // We can proceed with sending OTP.
      if (error.code !== 'auth/user-not-found') {
        // For other errors (e.g., network issues), re-throw them.
        console.error("Unexpected error checking for existing user:", error);
        throw error;
      }
    }
...
export async function sendEmailOtp(
  input: z.infer<typeof SendEmailOtpInputSchema>
): Promise<z.infer<typeof SendEmailOtpOutputSchema>> {
