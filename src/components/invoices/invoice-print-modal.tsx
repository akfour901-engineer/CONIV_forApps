...
export default function InvoicePrintModal({ isOpen, onOpenChange, invoice }: InvoicePrintModalProps) {
  const { userProfile } = useAuth();
  const [selectedLayout, setSelectedLayout] = useState('invoice-style');
  ...
  useEffect(() => {
    if (isOpen && invoice) {
      setSelectedLayout('invoice-style');
  ...
  const handleDownloadPdf = async () => {
    const element = document.getElementById('printable-invoice-content-desktop') ?? document.getElementById('printable-invoice-content-mobile');
    ...
    try {
        ...
        toast({ title: "Download Started", description: "Your invoice PDF is downloading." });
    } catch (error) {
     ...
     <div className="space-y-4">
        ...
        <div>
          ...
          <div className="space-y-1.5 text-sm">
            <div className="flex items-center space-x-2"><Checkbox id="showInvoiceMyContactDetailsModal" checked={showMyContactDetails} onCheckedChange={(checked) => setShowMyContactDetails(Boolean(checked))} /><Label htmlFor="showInvoiceMyContactDetailsModal" className="font-normal cursor-pointer text-xs">Show My Contact Details</Label></div>
            <div className="flex items-center space-x-2"><Checkbox id="showInvoiceBankDetailsModal" checked={showBankDetails} onCheckedChange={(checked) => setShowBankDetails(Boolean(checked))} /><Label htmlFor="showInvoiceBankDetailsModal" className="font-normal cursor-pointer text-xs">Include Bank Account</Label></div>
            <div className="flex items-center space-x-2"><Checkbox id="addInvoiceWatermarkModal" checked={addWatermark} onCheckedChange={(checked) => setAddWatermark(Boolean(checked))} /><Label htmlFor="addInvoiceWatermarkModal" className="font-normal cursor-pointer text-xs">Add Company Watermark</Label></div>
            <div className="flex items-center space-x-2"><Checkbox id="showInvoiceSignatureAreaModal" checked={showSignatureArea} onCheckedChange={(checked) => { const isChecked = Boolean(checked); setShowSignatureArea(isChecked); if (!isChecked) setAddDigitalSignature(false);}}/><Label htmlFor="showInvoiceSignatureAreaModal" className="font-normal cursor-pointer text-xs">Show Signature Area</Label></div>
            <div className="flex items-center space-x-2 pl-4"><Checkbox id="addInvoiceDigitalSignatureModal" checked={addDigitalSignature} onCheckedChange={(checked) => setAddDigitalSignature(Boolean(checked))} disabled={!showSignatureArea}/><Label htmlFor="addInvoiceDigitalSignatureModal" className="font-normal cursor-pointer text-xs">Add My Digital Signature</Label></div>
          </div>
        </div>
      </div>
  );
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl h-screen flex flex-col p-0 print:p-0">
        <DialogHeader className="print:hidden p-4 sm:p-6 pb-2 shrink-0">
          <DialogTitle className="flex items-center"><Printer className="mr-2 h-5 w-5 text-primary" /> Invoice Preview & Download</DialogTitle>
          <DialogDescription>
            Customize your invoice layout and content before downloading.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="flex flex-col flex-grow">
             ...
             <>
              <div className="flex-1 overflow-auto -mx-4 -my-4 bg-gray-50 p-2">
                 <div id="printable-invoice-content-mobile" className="mx-auto w-[210mm] min-h-[297mm] print:min-h-0">
                   <InvoicePrintLayout
                    key={previewKey}
                    invoice={invoice}
        ...
        <div className="hidden md:grid grid-cols-[280px_1fr] flex-1 overflow-hidden px-6 pb-6">
          ...
          <div className="flex-1 p-2 bg-gray-50 overflow-auto border rounded-md shadow-inner">
            <div id="printable-invoice-content-desktop" className="mx-auto w-[210mm] min-h-[297mm] print:min-h-0">
                <InvoicePrintLayout
                  key={previewKey}
                  invoice={invoice}
                  userProfile={userProfile}
