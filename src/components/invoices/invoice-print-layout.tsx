...
import type { BankAccount, UserProfile, Invoice } from '@/types';
...
export default function InvoicePrintLayout({
  ...
  useEffect(() => {
    const fetchBankAccount = async () => {
      ...
      if (showBankDetails && ownerId && invoice?.companyId && user) {
        setIsLoadingBankAccount(true);
        setBankAccount(null);
        try {
          const idToken = await user.getIdToken();
          const response = await fetch(`/api/bank-accounts/default-for-company?ownerId=${ownerId}&companyId=${invoice.companyId}`, {
          ...
          if (response.ok) {
            ...
            console.warn(`Could not fetch default bank account for company ${invoice.companyId}. Status: ${response.status}`);
            setBankAccount(null);
          }
        } catch (error) {
          console.error("InvoicePrintLayout: Error fetching bank account via API:", error);
          setBankAccount(null);
        } finally {
    ...
    if (showBankDetails && invoice && user) {
      fetchBankAccount();
    }
  }, [showBankDetails, invoice, setIsLoadingBankAccount, user]);
  ...
  if (!invoice) {
    return <div className="p-8 text-center text-red-500">Invoice data is missing.</div>;
  }
  ...
  return (
    <div className={cn(styles.page, "relative overflow-hidden")}>
      ...
      <div className="relative z-10">
        <header id="invoice-print-header-content" className={styles.header}>
          <div>
            {invoice.companyLogoUrl && (invoice.companyLogoUrl.startsWith('data:image') || invoice.companyLogoUrl.startsWith('http')) ? (
              <Image src={invoice.companyLogoUrl} alt={`${invoice.companyName || 'Company'} Logo`} width={120} height={60} className={cn("object-contain w-auto mb-2", styles.companyLogoHeight)} data-ai-hint="company logo"/>
            ...
            <h1 className="text-lg font-bold text-gray-800">{invoice.companyName || "Your Company Name"}</h1>
            {invoice.companyAddress && <p className="text-xs text-gray-600 whitespace-pre-line">{invoice.companyAddress}</p>}
            {invoice.companyGstin && <p className="text-xs text-gray-600">GSTIN: {invoice.companyGstin}</p>}
          </div>
          <div className="text-right">
            <h2 className={styles.mainTitle}>Invoice</h2>
            <p className="text-md text-gray-700 mt-1"># {invoice.invoiceNumber}</p>
        ...
        <section className="flex flex-col md:flex-row gap-4 mb-4 pb-4 border-b">
          <div className="flex-1 min-w-0">
            ...
            <p className="font-bold text-gray-800">{invoice.organizationName}</p>
            {invoice.organizationAddress && <p className="text-xs text-gray-600 whitespace-pre-line">{invoice.organizationAddress}</p>}
            {invoice.organizationGstin && <p className="text-xs text-gray-600">GSTIN: {invoice.organizationGstin}</p>}
          </div>
        </section>
        ...
        <section className="flex justify-end mb-4 page-break-inside-avoid">
          <div className="w-full md:w-2/5 lg:w-1/3 text-sm">
            <div className="flex justify-between py-1.5 border-b"><span>Subtotal:</span> <span className="font-medium">{formatCurrency(invoice.subTotal)}</span></div>
            {(invoice.taxRate != null && invoice.taxRate > 0) && (<div className="flex justify-between py-1.5 border-b"><span>Tax ({invoice.taxRate}%):</span> <span className="font-medium">{formatCurrency(invoice.taxAmount)}</span></div>)}
            <div className="flex justify-between py-2 text-md font-bold bg-secondary/50 px-2 rounded-sm"><span className="uppercase text-primary">Grand Total:</span> <span>{formatCurrency(invoice.grandTotal)}</span></div>
            {(invoice.amountPaid != null && invoice.amountPaid > 0) && (<div className="flex justify-between py-1.5 border-t mt-2"><span>Amount Paid:</span><span className="font-medium text-green-600">-{formatCurrency(invoice.amountPaid)}</span></div>)}
            <Separator className="my-1"/>
            <div className="flex justify-between text-lg font-bold text-destructive"><span>Balance Due:</span> <span>{formatCurrency(invoice.balanceDue)}</span></div>
          </div>
        </section>
        <footer className="mt-6 pt-4 border-t text-xs text-gray-600 space-y-4">
          <div className="flex flex-row gap-4 w-full">
            ...
            {invoice.notes && <div className="flex-1"><h4 className="font-semibold mb-1 text-gray-700">Notes:</h4><p className="whitespace-pre-wrap">{invoice.notes}</p></div>}
          </div>
          {showBankDetails && (
            <div>
          ...
          {invoice.paymentInstructions && <div className="mt-4"><h4 className="font-semibold mb-1 text-gray-700">Payment Instructions:</h4><p className="whitespace-pre-wrap">{invoice.paymentInstructions}</p></div>}
          {showSignatureArea && (
