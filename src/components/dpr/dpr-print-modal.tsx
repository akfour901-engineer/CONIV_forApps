...
export default function DprPrintModal({ isOpen, onOpenChange, report }: DprPrintModalProps) {
  ...
  // Fetch company and work order data (with companyId fallback)
  useEffect(() => {
    if (!isOpen || !report || !user) return;
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        let companyIdToUse = report.companyId;

        // IMPORTANT FIX: Fallback if companyId is missing in old DPRs
        if (!companyIdToUse && report.workOrderId) {
          console.warn(`DPR ${report.id} is missing companyId. Fetching from WorkOrder...`);
          
          const woFallbackRes = await fetch(`/api/work-orders/${report.workOrderId}`, {
            headers: { 'Authorization': `Bearer ${idToken}` }
          });

          if (woFallbackRes.ok) {
            const woData: WorkOrder = await woFallbackRes.json();
            companyIdToUse = woData.companyId;
          }
        }

        if (!companyIdToUse) {
          throw new Error("Could not determine Company ID for this DPR. Please contact support.");
        }

        // Fetch both WorkOrder and Company in parallel
        const [woResponse, compResponse] = await Promise.all([
          fetch(`/api/work-orders/${report.workOrderId}`, {
            headers: { 'Authorization': `Bearer ${idToken}` }
          }),
          fetch(`/api/companies/${companyIdToUse}`, {
            headers: { 'Authorization': `Bearer ${idToken}` }
          })
        ]);

        if (woResponse.ok) {
          setWorkOrder(await woResponse.json());
        } else {
          console.error("Failed to fetch work order");
        }

        if (compResponse.ok) {
          setCompany(await compResponse.json());
        } else {
  ...
  const handleDownloadPdf = async () => {
    const element = document.getElementById('printable-dpr-content-desktop') ??
                   document.getElementById('printable-dpr-content-mobile');

    if (!element) {
      toast({ title: "Download Error", description: "Could not find report content.", variant: "destructive" });
      return;
    }
  ...
  const OptionsPanel = () => (
    <div className="space-y-4">
      <div>
        <h3 className="font-semibold mb-2 text-md flex items-center">
          <PenLine className="mr-2 h-4 w-4 text-primary" /> Print Options
        </h3>
        <div className="space-y-1.5 text-sm">
          <div className="flex items-center space-x-2">
            <Checkbox id="addDprWatermark" checked={addWatermark} onCheckedChange={(checked) => setAddWatermark(Boolean(checked))} />
            <Label htmlFor="addDprWatermark" className="font-normal cursor-pointer text-xs">Add Company Watermark</Label>
          </div>
          <div className="flex items-center space-x-2">
            <Checkbox id="showDprSignatureArea" checked={showSignatureArea} onCheckedChange={(checked) => {
              const isChecked = Boolean(checked);
              setShowSignatureArea(isChecked);
              if (!isChecked) setAddDigitalSignature(false);
            }} />
            <Label htmlFor="showDprSignatureArea" className="font-normal cursor-pointer text-xs">Show Signature Area</Label>
          </div>
          <div className="flex items-center space-x-2 pl-4">
            <Checkbox id="addDprDigitalSignature" checked={addDigitalSignature} onCheckedChange={(checked) => setAddDigitalSignature(Boolean(checked))} disabled={!showSignatureArea} />
            <Label htmlFor="addDprDigitalSignature" className="font-normal cursor-pointer text-xs">Add My Digital Signature</Label>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent 
        className="max-w-4xl h-full flex flex-col p-0 print:p-0"
        aria-describedby={undefined}   // Fixed warning
      >
        <DialogHeader className="print:hidden p-4 sm:p-6 pb-2 shrink-0">
          <DialogTitle className="flex items-center">
            <Printer className="mr-2 h-5 w-5 text-primary" /> 
            DPR Preview & Download
          </DialogTitle>
        </DialogHeader>
          ...
          {mobileView === 'options' ? (
            ...
            <>
              ...
              <DialogFooter className="mt-4 flex-col sm:flex-row gap-2">
                <Button variant="outline" onClick={() => setMobileView('options')} className="w-full">
                  <ArrowLeft className="mr-2 h-4 w-4" /> Back
                </Button>
                <Button onClick={handleDownloadPdf} className="w-full" disabled={isDownloading || isLoading}>
                  {isDownloading ? (
