...
import ViewWorkOrderPageLoadingSkeleton from '@/app/dashboard/work-orders/[id]/loading';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2 w-full sm:w-auto">
          ...
          >
            <Link href={`/dashboard/work-orders/${workOrder.id}/edit`} className="flex items-center">
      ...
      <Card className="shadow-lg">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6 items-start">
            {workOrder.termsAndConditions && (<div className="text-sm"> <h3 className="font-semibold mb-1 text-primary">Terms & Conditions</h3> <p className="text-muted-foreground whitespace-pre-wrap">{workOrder.termsAndConditions}</p> </div>)} <div className={!workOrder.termsAndConditions ? "md:col-start-2" : ""}> <Card className="bg-secondary/50 p-4 text-sm"> <CardContent className="space-y-1 p-0"> <div className="flex justify-between"><span>Subtotal:</span> <span className="font-medium">{formatCurrency(workOrder.subTotal)}</span></div> {(workOrder.taxRate !== undefined && workOrder.taxRate !== null && workOrder.taxRate > 0) && (<div className="flex justify-between"><span>Tax ({workOrder.taxRate}%):</span> <span className="font-medium">{formatCurrency(workOrder.taxAmount ?? undefined)}</span></div>)} <Separator className="my-1 bg-border"/> <div className="flex justify-between text-md font-bold text-primary"><span>Grand Total:</span> <span>{formatCurrency(workOrder.grandTotal)}</span></div> </CardContent> </Card> </div>
          </div>
        </CardContent>
