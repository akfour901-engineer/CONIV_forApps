...
export default function AuditClientPage() {
    ...
    const [sdFilter, setSdFilter] = useState<'all' | 'with_sd' | 'without_sd'>('all');
    const [licenseFilter, setLicenseFilter] = useState<'all' | 'expiring_soon'>('all');
    ...
    const handleAnalyze = async () => {
        ...
        const auditInput: AuditContractorActivitiesInput = {
            ...
            sdFilter: sdFilter as any,
            licenseFilter: licenseFilter as any,
            userId: dataOwnerId,
            actorUid: user.uid,
        ...
        try {
            ...
            const result: AuditContractorActivitiesOutput = await response.json();
                ...
                <CardContent className="space-y-4">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                           ...
                           <div><Label>Security Deposit</Label><Select value={sdFilter} onValueChange={(value) => setSdFilter(value as 'all' | 'with_sd' | 'without_sd')}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="all">All</SelectItem><SelectItem value="with_sd">With SD</SelectItem><SelectItem value="without_sd">Without SD</SelectItem></SelectContent></Select></div>
                           <div><Label>Licenses</Label><Select value={licenseFilter} onValueChange={(value) => setLicenseFilter(value as 'all' | 'expiring_soon')}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="all">All</SelectItem><SelectItem value="expiring_soon">Expiring Soon</SelectItem></SelectContent></Select></div>
                        </div>
                    </div>
                </CardContent>
                <CardFooter>
