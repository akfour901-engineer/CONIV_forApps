...
import AiFraudDetectorLoading from '@/app/dashboard/advance-tools/ai-fraud-detector/loading';
import { Badge } from '@/components/ui/badge';
import type { SuspiciousActivity } from '@/types/server-only';
...
export default function AiFraudDetectorClientPage() {
    ...
    const [analysisResult, setAnalysisResult] = useState<Omit<FraudDetectionOutput, 'newResourcePoints'> | null>(null);
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            const response = await fetch('/api/ai/detect-fraud', {
                ...
                body: JSON.stringify({ dataOwnerId: dataOwnerId, actorUid: user.uid, actorName: userProfile.fullName }),
            });
            const result: FraudDetectionOutput = await response.json();
            if (!response.ok) throw new Error(result.error || 'Failed to get fraud analysis.');
