...
import type { Company, Organization, Estimate, EstimateItem, WorkOrder, WorkOrderItem, WorkOrderStatus, SorRate, AppConfiguration, AISuggestedEstimateItem } from '@/types/server-only';
...
const DEFAULT_WO_ITEMS_FORM = [{ itemCode: "", description: "", quantity: 1, unit: "nos", rate: 0, id: 'default-0', amount: 0 }];
...
function NewWorkOrderPageContent() {
  ...
  const fetchPrerequisitesAndPrefill = useCallback(async () => {
    ...
    try {
        ...
        if (!response.ok) throw new Error('Failed to fetch required data.');
        ...
        if (templateEstimateId) {
            ...
            if (template) {
                form.reset({
                    ...
                    items: template.items.map((item: WorkOrderItem) => ({...item, id: undefined, amount: item.rate * item.quantity })),
                    taxRate: template.taxRate,
                    scopeOfWork: template.subjectOfWork,
                    termsAndConditions: template.termsAndConditions,
                    estimateId: template.id,
                });
                toast({ title: "Template Loaded", description: "Work Order details pre-filled from approved estimate." });
            }
        }
    } catch (error: any) {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <Card>
              ...
              <CardFooter><Button type="submit" disabled={isSubmittingForm || authLoading || !canCreateWorkOrders}>{isSubmittingForm ? <><Loader2 className="mr-2 h-4 w-4 animate-spin"/>Creating...</> : <><Save className="mr-2 h-4 w-4"/>Create WO</>}</Button></CardFooter>
            </Card>
          </form>
        </Form>
        )}
