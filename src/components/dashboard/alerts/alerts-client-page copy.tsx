    ...
    if ('Notification' in window) {
      Notification.requestPermission().then(permission => {
        ...
        if (permission === 'granted') {
          toast({ title: "Notifications Enabled!", description: "You'll now receive real-time alerts." });
        } else {
          toast({ title: "Notifications Blocked", description: "You can enable them from your browser settings.", variant: "destructive" });
        }
      });
    }
  };
  ...
  const fetchAlerts = useCallback(async () => {
    ...
    try {
        ...
        if (previousAlertsRef.current.length > 0 && newAlertsData.length > previousAlertsRef.current.length) {
            ...
            if (newAlerts.length > 0 && 'Notification' in window && Notification.permission === 'granted' && navigator.serviceWorker.ready) {
                navigator.serviceWorker.ready.then((registration) => {
                     if (newAlerts.length === 1) {
                         const notification = newAlerts[0];
                         registration.showNotification(notification.title, {
                             body: notification.description,
                             icon: '/icons/icon-192.png',
                             badge: '/icons/icon-192.png',
                             data: { url: notification.href },
                             tag: notification.id,
                         });
                     } else {
                         registration.showNotification(`${newAlerts.length} New Alerts`, {
                             body: `You have new updates, including: "${newAlerts[0].title}"`,
                             icon: '/icons/icon-192.png',
                             badge: '/icons/icon-192.png',
                             data: { url: '/dashboard/alerts' },
                             tag: 'new-alerts-summary',
                         });
                     }
                 });
            }
        ...
        previousAlertsRef.current = newAlertsData; // Update baseline for next fetch
    } catch (error: any) {
        ...
        <CardHeader>
          ...
          <div className="pt-2 flex flex-col md:flex-row gap-2">
            ...
            <Select value={typeFilter} onValueChange={(value) => setTypeFilter(value as 'all')}>
                <SelectTrigger className="w-full md:w-[220px]"><SelectValue placeholder="Filter by Type" /></SelectTrigger>
