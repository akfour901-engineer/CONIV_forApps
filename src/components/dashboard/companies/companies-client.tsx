...
const isSvgString = (str: string | null | undefined): boolean => {
  return typeof str === 'string' && str.trim().startsWith('<svg');
};

const getLogoSrc = (logoUrl: string | null | undefined) => {
  if (isSvgString(logoUrl)) {
    if (typeof window !== 'undefined') {
      return `data:image/svg+xml;base64,${window.btoa(logoUrl!)}`;
    }
    // SSR fallback. next/image will likely error on the server with this, but client-side will fix it.
    return `data:image/svg+xml,${encodeURIComponent(logoUrl!)}`;
  }
    ...
    <Card key={company.id} className="shadow-md hover:shadow-lg transition-shadow flex flex-col h-full">
      <CardHeader className="flex flex-row items-start gap-4 space-y-0 pb-2">
        <div className="relative w-16 h-16 shrink-0">
          <Image src={logoSrc} alt={`${company.name} logo`} layout="fill" objectFit="contain" className="rounded-md border p-1" data-ai-hint="company logo" />
        </div>
