...
import EmailLogsLoadingSkeleton from '@/app/dashboard/email-logs/loading';
...
export default function EmailLogsClientPage() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      // This API route will fetch logs for the user's own email.
    ...
    setIsLoading(false);
  }, [dataOwnerId, user, toast, setIsLoading]);
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold flex items-center">
            <Mail className="mr-3 h-7 w-7 text-primary" /> My Email Log
          </h1>
          <p className="text-muted-foreground">History of emails sent from your account.</p>
        </div>
              ...
              <Table>
                ...
                <TableBody>
                  {paginatedLogs.length > 0 ? (
                    ...
                    <TableRow><TableCell colSpan={4} className="text-center h-24">No email logs found.</TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
            </div>
