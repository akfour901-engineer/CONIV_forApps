...
import type { Company, Organization, WorkOrder, SorRate, TeamPermissions, UserProfile } from '@/types';
...
async function checkPermissions(authenticatedUserUid: string, requestedDataOwnerId: string): Promise<boolean> {
  ...
  if (userProfile.ownerId === requestedDataOwnerId) {
    ...
    if (teamMemberDocSnap.exists) {
      ...
      // Allow if user has permission to create invoices (which requires this data)
...
export async function GET(request: Request) {
  const functionCallId = `api_invoice_form_data_GET_${Date.now()}`;
  try {
    ...
    const [companiesSnap, userOrgsSnap, publicOrgsSnap, workOrdersSnap, userSorRatesSnap, publicSorSnap] = await Promise.all([
      ...
      adminDb.collection('workOrders').where('userId', '==', dataOwnerId).get(),
    ...
    console.error(`[${functionCallId}] Error fetching form data for invoice:`, error);
