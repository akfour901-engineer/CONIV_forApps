...
import { formatCurrency, formatDate, isBefore, addDays } from '@/lib/utils';
  ...
  try {
            ...
            try {
                ...
                if (notificationPrefs.weeklySecurityDepositFollowups) {
                    if ((data.securityDeposit ?? 0) > 0 && data.depositPeriod !== undefined && data.depositPeriod !== null) {
                        ...
                        if (isBefore(sdDueDate, addDays(today, 90))) {
    ...
    if(hasPermission('canManageOrganizations') && notificationPrefs.newMessages) { // Assuming 'newMessages' covers CRM type alerts
      ...
      leadSnap.forEach(doc => {
        ...
        if (isBefore(updatedAt, fourteenDaysAgo)) {
    ...
    if (hasPermission('canManageOwnerLicenses') && notificationPrefs.importantUpdates) {
      ...
      licSnap.docs.forEach(d => {
        ...
        if(isBefore(parseISO(lic.expiryDate), addDays(today, 90)) && !isBefore(parseISO(lic.expiryDate), today)) {
    ...
    if (hasPermission('canManageLabourRegister') && notificationPrefs.importantUpdates) {
        ...
        labourSnap.forEach(doc => {
            ...
            const checkExpiry = (dateStr: string | undefined | null, docType: string) => {
                if(dateStr && isBefore(parseISO(dateStr), addDays(today, 90)) && !isBefore(parseISO(dateStr), today)) {
    ...
    if (notificationPrefs.marketplaceUpdates) {
        ...
        listingsSnap.forEach(doc => {
            ...
            if (isBefore(expiryDate, addDays(today, 7)) && !isBefore(expiryDate, today)) {
    ...
    if (hasPermission('canManageTeam') && notificationPrefs.newMessages) {
        ...
        const recentPermissionLogs = permissionsLogSnap.docs
            .map(doc => doc.data() as ActivityLog)
            .filter(log => log.timestamp && !isBefore(parseISO(log.timestamp), sevenDaysAgo))
            .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());

        recentPermissionLogs.forEach(log => {
            fetchedAlerts.push({ id: `perm-log-${log.id}`, title: `Team Permissions Changed`, description: `${log.actorName} updated permissions for ${log.entityName}.`, href: `/dashboard/team`, icon: 'UserCog', type: 'team', date: log.timestamp });
        });
    }
