...
import type { Company, Organization, WorkOrder, SorRate, TeamPermissions, UserProfile, Estimate } from '@/types';
...
async function checkPermissions(authenticatedUserUid: string, requestedDataOwnerId: string): Promise<boolean> {
  ...
  if (userProfile.ownerId === requestedDataOwnerId) {
    ...
    if (teamMemberDocSnap.exists) {
      ...
      // Allow if user has permission to create work orders (which requires this data)
...
export async function GET(request: Request) {
  const functionCallId = `api_wo_form_data_GET_${Date.now()}`;
  try {
    ...
    const [companiesSnap, orgsSnap, estimatesSnap, sorRatesSnap, publicSorSnap] = await Promise.all([
      adminDb.collection('companies').where('userId', '==', dataOwnerId).get(),
      adminDb.collection('organizations').where('userId', '==', dataOwnerId).get(),
      adminDb.collection('estimates').where('userId', '==', dataOwnerId).where('status', '==', 'approved').get(),
    ...
    console.error(`[${functionCallId}] Error fetching form data for WO:`, error);
